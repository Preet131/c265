...
def make_grayscale(image):
    imageArray = np.fromstring(image, dtype='uint8')
    print("Image Array:", imageArray)
    decodeArrayToImage = cv2.imdecode(imageArray, cv2.IMREAD_UNCHANGED)
    print("Decode Values of Image:", decodeArrayToImage)

    convertedGrayImage = cv2.cvtColor(decodeArrayToImage, cv2.COLOR_RGB2GRAY)

    status, outputImage = cv2.imencode('.PNG', convertedGrayImage)
    print('Status:', status)
